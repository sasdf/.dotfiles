...
sync_tmp2_main () {
    ...
    if [ -d "$DOTS_PATH_LOCAL" ] ; then
        REMOTETS="$(ssh $DOTS_LOCAL_SYNC_SERVER "cat $DOTS_PATH_LOCAL/timestamp 2>/dev/null")" || return 0
        LOCALTS="$(cat "$DOTS_PATH_LOCAL/timestamp" 2>/dev/null)" || return 0
        CURRENTTS="$(date +%s)" || return 0
        if [ -z "$REMOTETS" ]; then
